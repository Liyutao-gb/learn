				...
				<div style="position: absolute;top: -14px;left: 241px;">
					<form class="navbar-form navbar-right" role="search" @submit.prevent="pageHandler()">
						<div class="form-group">
							<input type="text" class="form-control" v-model="searchContent" name="content" placeholder="文件名称">
						</div>
				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						<tr v-for="(courseFile,index) in courseFileList" class="showStyle">
							...
							<c:choose>
								<c:when test="${sessionScope.role==0}">
									<td><button type="button" class="btn btn-danger" href="javascript:void()" @click="deleteBtn(courseFile.id)">删除</button></td>
								</c:when>
								<c:when test="${sessionScope.role==1}">
									<td><button type="button" class="btn btn-danger" href="javascript:void()" @click="deleteBtn(courseFile.id)">删除</button></td>
								</c:when>
		...
		var courseVue=new Vue({
			...
			methods:{
				pageHandler:function(page){//跳转到page页
					...
					var paramsTerm={"page":1,"pageSize":this.pageSize,"content":content};
					//条件查询
				...
				download:function(path){
					 location.href = "/learn/upload/" + path;
				},
