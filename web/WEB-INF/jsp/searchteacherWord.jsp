...
<html>
<head>
	<title>单词学习[管理]-在线学习平台ForFuture</title>
	...
	<div class="row">
		<div class="col-lg-12">
			<ul class="breadcrumb wk-breadcrumb">
	            ...
	            <li><a href="javascript:void(0);">单词学习管理</a></li>
			</ul>
		</div>
	</div>
                ...
                <div class="panel-body panel-button">
	                ...
	                <c:choose>
						<c:when test="${sessionScope.role==2}">
							<!--  若为学生 移除  -->
							<button id="deleteBtn" type="button" class="btn btn-default wk-tool-btn">移出生词本</button>
						</c:when>
						<c:when test="${sessionScope.role==1}">
							<!--  若为教师  则是管理自己发布的 -->
							<button id="deleteBtn2" type="button" class="btn btn-default wk-tool-btn">删除</button>
	                		<button id="updateBtn" type="button" class="btn btn-default wk-tool-btn">编辑</button>
	                		<button id="addBtn" type="button" class="btn btn-default wk-tool-btn">添加单词</button>
						</c:when>
					</c:choose>
                 </div>
            </div>
		...
		<div class="col-lg-12">
			<div class="panel panel-default  wk-panel">
				<table class="table table-striped table-hover">
					...
					<tbody>
						<tr v-for="(word,index) in wordList" class="showStyle">
							<td>
								<input :id="'grid_'+(word.id)" name="workerRadio" type="radio" aria-label="选择" />
							</td>
							...
							<td>{{word.sentence}}</td>
							<td>{{word.belongTo}}</td>
							<td>{{dateFormat(word.createTime)}}</td>
						</tr>
					</tbody>
	...
	<div class="modal fade" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">您确定要移除这些记录？</div>
	...
	<script>
	    ...
	    $("#deleteBtn").on("click", function() {
	        ...
	        if (id != "") {
	            ...
	            $("#deleteBtnOK").on("click", function() {
	                location.href = "<%=basePath%>deleteWord/" + id + ".html";
	            });
	        }else{
        		alert("请选择您需要删除的单词！！");
     		}
	    ...
	    $("#deleteBtn2").on("click", function() {
	        ...
	        if (id != "") {
	            $("#deleteConfirm").modal();
	            $("#deleteBtnOK").on("click", function() {
	            	location.href = "<%=basePath%>deleteTeacherWord/" + id + ".html";
	            });
	        }else{
        		alert("请选择您需要删除的单词！！");
     		}
		...
		/* 更新讨论话题 */
	     $("#updateBtn").on("click", function() {
	        ...
	        if (id != "") {
				location.href = "<%=basePath%>updateWord/" + id + ".html";
	        }else{
        		alert("请选择您需要更改的单词！！");
     		}
	    });
	     ...
	     $("#addBtn").on("click", function() {
			...
			location.href = "<%=basePath%>createWord.html";
	     });
		...
		var discussVue=new Vue({
			...
			methods:{
				pageHandler:function(page){//跳转到page页
					...
					//
					this.$http.post("teacherWordListByPage.action",params,{emulateJSON:true}).then(
						function(res){
